...
describe('Auth Middleware - verifyToken', () => {
  ...
  describe('Invalid or expired tokens', () => {
    it('should return 401 for invalid token and log error', () => {
      ...
      expect(logger.error).toHaveBeenCalledWith('Error occurred at auth middleware: ', jwtError);
    ...
    it('should return 401 for expired token', () => {
      ...
      expect(logger.error).toHaveBeenCalledWith('Error occurred at auth middleware: ', expiredError);
      expect(next).not.toHaveBeenCalled();
    });
    ...
    it('should return 401 for malformed token', () => {
      ...
      expect(logger.error).toHaveBeenCalledWith('Error occurred at auth middleware: ', malformedError);
      expect(next).not.toHaveBeenCalled();
    });
  });
  ...
  describe('Edge cases', () => {
    ...
    it('should handle header with Basic auth format that has space', () => {
      ...
      expect(jwt.verify).toHaveBeenCalledWith('username:password', 'test-secret-key');
      expect(req.user).toEqual(mockDecoded);
      expect(next).toHaveBeenCalledTimes(1);
    });
  });
  ...
  describe('Environment variable handling', () => {
    it('should handle missing JWT_SECRET environment variable', () => {
      ...
      expect(logger.error).toHaveBeenCalledWith('Error occurred at auth middleware: ', secretError);
    });
  });
